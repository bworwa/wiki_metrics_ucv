				...
				if command_list[0] == "clear":
					...
					try:
						...
						if command_list[1] == "-a":
							...
							priority_daemon_pid = Daemon(None).get_pid(Priority_Daemon.config["pid_file_path"])
							...
							wikimetrics_daemon_pid = Daemon(None).get_pid(Wikimetrics_Daemon.config["pid_file_path"])
